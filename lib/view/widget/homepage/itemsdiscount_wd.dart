...
class ItemsDiscountWidget extends StatelessWidget {
  const ItemsDiscountWidget({super.key});
    ...
    return Skeletonizer(
      ...
      child: SizedBox(
        ...
        child: ListView.separated(
          ...
          itemCount: isLoading ? 4 : homePageControllerImp.itemsdiscount.length,
          itemBuilder: (context, i) {
            return ItemsDiscount(
              itemsModel: isLoading
                  ? null
                  : ItemsModel.fromJson(homePageControllerImp.itemsdiscount[i]),
            );
          },
        ),
      ),
...
class ItemsDiscount extends StatelessWidget {
  const ItemsDiscount({super.key, required this.itemsModel});
  final ItemsModel? itemsModel;

  @override
  Widget build(BuildContext context) {
    HomePageControllerImp controller = Get.find();
    bool isLoading = controller.statusRequest == StatusRequest.loading;
